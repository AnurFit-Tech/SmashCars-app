...
using Market.Application.Modules.Catalog.Discount.Commands.Create;
...
public class ProductsController(ISender sender) : ControllerBase
{
    ...
    [HttpPut("inline/{id:int}")]
    public async Task UpdateInline(int id, UpdateInlineProductCommand command, CancellationToken ct)
    {
        // ID iz rute ima prioritet
        command.Id = id;
        await sender.Send(command, ct);
        // nema povratne vrijednosti -> 204 No Content
    }
    ...
    [HttpPost("discounts")]
    public async Task CreateDiscount([FromBody] CreateDiscountCommand command, CancellationToken ct)
    {
        await sender.Send(command, ct);
    }
}